!code name: cn_gp_1d.f90
!
!description: This code solves the Gross-Pitaevskii(GP)
!             equation by Crank-Nicolson (CN)
...
!             solution.
!
!     Problem: Bose-Einstein condensate (BEC) harmonically trapped
!
!            i*ut=-0.5*uxx+Vtrap(x)*u+g*|u|**2*u, Vtrap(x)=0.5*x**2
!
!           We want to find the results of the ground-state, based
!           on the tableII from ref.(1)
!
!                        g      mu
!                        __________
!
...
!     references:
!
!           1) https://journals.aps.org/pra/abstract/10.1103/PhysRevA.93.053612
!
...
real*8,parameter::Lx=20.0d0  !space
...
complex*16,parameter::ci=(0.0d0,1.0d0), cr=(1.0d0,0.0d0) !useful complex numbers
real*8,parameter:: g=1.0d0 !interaction parameter
...
open(100,file="wave_density.txt")
do i=1,Nx+1
   write(100,*)x(i),dreal(dconjg(psi(i))*psi(i))
end do
open(200,file="energies.txt")
write(200,*)'Energy: ',E
write(200,*)'Chemical potential: ',mu
...
do k=1,Nt
  ...
  !energy convergence criterion to finish the loop
   ...
   error=dabs(E-Eold)
   Eold=E
   write(1,991)k*Ntt*dt,E,mu,error,res,Vir
...
do k=1,Nt
   ...
   call energy(psi,E,mu,res,vir)
   call norm(psi,norma)
   write(2,992)k*Ntt*dt,norma,E,mu
end do
...
use variables, only:Nx,Vtrap,ci,cr,g
implicit none
complex*16,dimension(1:Nx+1)::psi !wave function
...
do i=1,Nx+1
   V(i)=Vtrap(i)+g*dconjg(psi(i))*psi(i)
end do
...
do i=1,Nx+1
      ...
      !interaction energy
      Eint=Eint+(fx(i)*dx*0.5d0*g*denp(i)**2)/3.0d0
end do
!total energy
E=Epot+Ekin+Eint
!chemical potential
mu=Epot+Ekin+2*Eint
!virial theorem
vir=dabs(-Ekin+Epot-0.5d0*Eint)

!residue
do i=2,Nx+1
   res_vec(i)=-0.5d0*(psi(i+1)-2*psi(i)+psi(i-1))/(dx**2) &
             &+Vtrap(i)*psi(i) &
             &+dconjg(psi(i))*psi(i)*psi(i)&
             &-mu*psi(i)
end do
