...
#
# obs: this version write the file using a numpy command
#
#date: June 29th, 2021
...
data1 = np.empty([x.size, 2])
data1[:,0] = x
data1[:,1] = psi
np.savetxt('heat_cn_new.txt', data1, fmt="%.14E")        

data2 = np.empty([x.size, 2])
data2[:,0] = x
data2[:,1] = y
np.savetxt('heat_exact_new.txt', data2, fmt="%.14E")        
